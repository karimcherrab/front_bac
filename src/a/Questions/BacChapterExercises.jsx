    ...
    <MathJaxContext version={3} config={MATHJAX_CONFIG}>
      ...
      <section
        ...
        className="
          ...
          min-w-0
          overflow-x-hidden
          bg-[radial-gradient(circle_at_top_right,_#eef2ff_0,_#f8fafc_34%,_#f4f6f8_72%)]
          px-2
          py-3
          min-[360px]:px-3
          min-[360px]:py-4
              ...
              <button
                ...
                className={cn(
                  ...
                  showFullSolution
                    ...
                    : "bg-gradient-to-l from-indigo-700 to-blue-700 text-white shadow-lg shadow-blue-900/10 hover:from-indigo-800 hover:to-blue-800"
                )}
              >
    ...
    <div className="min-w-0 space-y-5 sm:space-y-6">
      ...
      {steps.length > 0 && (
        <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between gap-3 border-b border-slate-200 bg-gradient-to-l from-slate-100 to-white px-4 py-4 sm:px-5">
            <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 space-y-4 bg-slate-50/60 p-3 min-[360px]:p-4 sm:p-5">
...
function SolutionStep({
  ...
  return (
    <article
      dir="rtl"
      className={cn(
        "relative min-w-0 overflow-hidden rounded-3xl border bg-white shadow-sm transition",
        indeterminateForm
    ...
    >
      {indeterminateForm && (
        <div className="flex items-center gap-2 border-b border-amber-200 bg-amber-50 px-4 py-2.5 sm:px-5">
          <TriangleAlert size={16} className="shrink-0 text-amber-700" />
          <span className="text-xs font-black text-amber-900">
            {indeterminateForm.label} — يجب رفعها قبل حساب النهاية
          </span>
        </div>
      )}

      <div className="flex min-w-0 items-start gap-3 px-4 py-4 sm:gap-4 sm:px-5 sm:py-5">
        {/* رقم الخطوة */}
        <div className="relative shrink-0">
          <div
            className={cn(
              "flex h-10 w-10 items-center justify-center rounded-2xl text-sm font-black text-white shadow-sm",
              indeterminateForm
                ? "bg-amber-500"
                : isLast
                  ? "bg-emerald-700"
          ...
          {!isLast && (
            <span className="absolute left-1/2 top-11 h-6 w-px -translate-x-1/2 bg-slate-200 sm:h-8" />
          )}
        </div>

        <div className="min-w-0 flex-1">
          {hasText(step?.title) && (
            <h4 className="mb-2 break-words text-base font-black leading-8 text-slate-950 sm:text-lg">
              {step.title}
            </h4>
          )}

          {hasText(step?.explanation) && (
            <RichExerciseText className="max-w-full font-medium leading-9 text-slate-700">
              {step.explanation}
            </RichExerciseText>
          )}
          ...
          {/* القانون */}
          {hasText(ruleUsed) && (
            <div className="mt-4 min-w-0 overflow-x-auto rounded-2xl border border-amber-100 bg-amber-50/80 px-4 py-3">
              <p className="mb-1 text-[11px] font-black text-amber-700">
                القاعدة المستعملة
              </p>
              <MathText block className="min-w-0 font-bold leading-8 text-amber-950">
                {ruleUsed}
              </MathText>
            </div>
          )}
          ...
          {/* لماذا؟ */}
          {hasText(why) && (
            <div className="mt-4 flex min-w-0 items-start gap-2 rounded-2xl border border-indigo-100 bg-indigo-50/70 px-4 py-3">
              <Lightbulb
                size={17}
          ...
          {/* نصيحة التلميذ - كانت موجودة في JSON ولم تكن تظهر */}
          {hasText(studentTip) && (
            <div className="mt-3 flex min-w-0 items-start gap-2 rounded-2xl border border-sky-100 bg-sky-50 px-4 py-3">
              <Sparkles
                size={16}
          ...
          {hasText(result) && (
            <div className="mt-4 flex min-w-0 items-start gap-2 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3">
              <CheckCircle2
                size={18}
...
function CalculationBox({ value, order = null }) {
  ...
  return (
    ...
    >
      {order !== null && (
        <span className="absolute left-2 top-2 flex h-6 min-w-6 items-center justify-center rounded-lg bg-slate-800 px-1.5 text-[10px] font-black text-white">
          {order}
        </span>
      )}
  ...
  return (
    ...
    >
      ...
      <div className="flex w-full items-center justify-center bg-white p-2 min-[360px]:p-3 sm:p-4">
        {loadError ? (
          <div className="flex w-full flex-col items-center justify-center rounded-xl border border-dashed border-amber-300 bg-amber-50 px-4 py-8 text-center">
            <TriangleAlert size={28} className="text-amber-700" />
            <p className="mt-3 text-sm font-black text-amber-950">
              تعذر تحميل الوثيقة العلمية
            </p>
            <p dir="ltr" className="mt-2 max-w-full break-all text-xs font-bold text-amber-800">
              {src}
            </p>
            <button
              type="button"
