...
import CoursePurchaseBanner from "../components/payments/CoursePurchaseBanner";
import LockedChapterCard from "../components/payments/LockedChapterCard";
...
export default function MathCoursePage() {
  ...
  const {
    ...
    subjectId: id_subjects,
    token,
    refreshKey,
  });

  const chapterPackById =
    useMemo(() => {
      const map = new Map();
  ...
  const subjectPack = useMemo(
    () =>
      paymentPacks.find(
        (pack) =>
          pack.pack_type === "subject",
      ) || null,
    [paymentPacks],
  );
  ...
  const chapterPacks = useMemo(
    () =>
      paymentPacks.filter(
        (pack) =>
          pack.pack_type === "chapter",
      ),
    [paymentPacks],
  );

  const openChapterOffer = (chapter) => {
    setOfferModal({
      open: true,
      chapter,
      defaultMode: "chapter",
  ...
  return (
    ...
    >
      ...
      <PurchaseOfferModal
        ...
        token={token}
        course={courseData}
        chapter={offerModal.chapter}
        chapterPack={
          offerModal.chapter?.pack || null
        }
        subjectPack={subjectPack}
        chapterPacks={chapterPacks}
        defaultMode={
          offerModal.defaultMode
        }
        onShowAllOffers={() => {
          closeOfferModal();
          navigate(
            `/pricing?subject=${id_subjects}${
              offerModal.chapter?.id
                ? `&chapter=${offerModal.chapter.id}`
                : ""
            }`,
          );
        }}
